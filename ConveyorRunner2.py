...
def pick_and_place_robot_dos():
    ...
    try:
        ...
        robot.MoveJ(targets[1], False)
        while robot.Busy(): time.sleep(0.01)

        robot.setSpeed(VEL_LENTA_LIN, VEL_LENTA_JNT)
        robot.MoveL(targets[2], False)
        while robot.Busy(): time.sleep(0.01)
        ...
        if contador == 0:
            target_destino = targets[4]
            offset_y = 0
        elif contador == 1:
            target_destino = targets[5]
            offset_y = 100
        elif contador == 2:
            target_destino = targets[6]
            offset_y = 200
        else:
            return
